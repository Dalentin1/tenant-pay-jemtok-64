...
import { addBill, addPayment, addSharedBill, deleteSharedBill, getTenantById, getUnpaidBillForTenant, addAnnouncement, updateTenant, addDelegate, deleteDelegate, updateBillStatus, getTenants, updateAllocatedBill, updateBill, updateAdminPassword, clearAllData } from '@/lib/data';
...
export async function adminLoginAction(prevState: any, formData: FormData) {
    ...
    if (password === process.env.ADMIN_PASSWORD) {
        ...
        cookies().set('admin_session', 'admin_logged_in', { expires, httpOnly: true });
        redirect('/admin');
    } else {
...
export async function clearAllDataAction() {
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred.';
        return { success: false, message: `Database Error: Failed to clear data. ${errorMessage}` };
    }
}
